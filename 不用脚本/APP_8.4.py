...
class DataShow(QWidget):
    ...
    def pdos_show_diy(self,str_name,lineEdit_x_l,lineEdit_x_r,lineEdit_y_l,lineEdit_y_r,linepara,hidden_edit,hidden_label):
            ...
            for segs in return_lst:
                ...
                out_pdos+="\"%s/%s\" u 1:%d w l lc %d lw %s t \"%s\","%(str_name,segs[1],int(segs[2]),return_lst.index(segs),line_width,segs[3])
                if spin_mode==1:
                    out_pdos+="\"%s/%s\" u 1:(-1*$%d) w l lc %d lw %s notitle,"%(str_name,segs[1],int(segs[2])+1,return_lst.index(segs),line_width)
                if fill_fermi=='1':
                    if spin_mode==1:
                        out_pdos+="\"%s/%s\" u 1:($1<0 ? $%d : 1/0):($1<0 ? -1*$%d : 1/0) with filledcurves lc %d notitle,"%(str_name,segs[1],int(segs[2]),int(segs[2])+1,return_lst.index(segs))
                    elif spin_mode==0:
                        out_pdos+="\"%s/%s\" u 1:($1<0 ? $%d : 1/0) with filledcurves x1 lc %d notitle,"%(str_name,segs[1],int(segs[2]),return_lst.index(segs))
            f_gnu.writelines("p"+out_pdos+"\n")
            f_gnu.close()
            #time.sleep(1)
            subprocess.call(['gnuplot', '-e', 'l \"gnu_buffer_pdos\"','--persist'])        
