...
input_seg=input_str.strip().split(";")#decode the input into [atomic_number,orbital_number,orbital_type]
atomic_buffer=[]
for i,v in enumerate(input_seg):
    atomic_buffer_seg=[]
    if "(" in v and ")" in v:
        numb_seg=v.split(")")[0].split("(")[1].split(",")
        ...
        raise ValueError(f"the segment {v} is invalid! plz type in (number-number,number,...)-oribit/element-orbit [orbit could be \"tot\"]")
...
for i in [x.split("-")[-1] for x in detected_dic]:
    ...
    if i=='tot':
        ...
        if orbit_proj[orbit_index][0]=='s':
            read_lines_from_pdos_lst.append(5)
        if orbit_proj[orbit_index][0]=='p':
            read_lines_from_pdos_lst.append(9)
        if orbit_proj[orbit_index][0]=='d':
            read_lines_from_pdos_lst.append(13)
    if draw_index_i==0:
...
if error==0:
    ...
    for i,v in enumerate(result_his_lst):
        for j,m in enumerate(detected_dic):
            if m==v[1]:
                print("read!",m,v[0],draw_index[j])
                found_file_index.append(j)

    count_output=0 
    original_files=len(result_his_lst)
    for j in range(len(atomic_dic_files)):
        if j not in found_file_index:
            print("Calculating...",detected_dic[j],draw_index[j])
