...
from PyQt5.QtWidgets import QApplication, QWidget, QHBoxLayout, QVBoxLayout, QTableWidget, QTableWidgetItem, QPushButton,QComboBox, QLabel, QTextEdit, QMainWindow, QCheckBox, QLineEdit, QScrollArea, QSizePolicy, QFileDialog
...
class AnalyzeShow(QMainWindow):
    ...
    def initUI(self):
        ...
        inner_layout_pdos_diy = QVBoxLayout(inner_widget_diy)
        ...
        linepara_pdos = QLineEdit("1;1;0;1")
        ...
        hidden_edit_pdos=QLineEdit("-1-s")
        ...
        splitter_pdos_diy.setWidget(inner_widget_diy)
...
class AnaVisual(QMainWindow):
    ...
    def initUI(self):
        ...
        layout_anavis = QVBoxLayout(central_widget)

        table_analyze = QTableWidget(self)
        ...
        data = [[value for value in row if value] for row in data_raw]
        #print(data)

        # 添加保存按钮
        save_button = QPushButton('保存表格数据')
        save_button.clicked.connect(self.saveTable)

        table_analyze.setRowCount(len(data))
        max_column=max([len(i) for i in data])
        table_analyze.setColumnCount(max_column+1)
        for i in range(len(data)):
            for j in range(len(data[i])):
                table_analyze.setItem(i, j, QTableWidgetItem(str(data[i][j])))
        table_analyze.resizeColumnsToContents()

        # 将表格和按钮添加到主布局中
        layout_anavis.addWidget(table_analyze)
        layout_anavis.addWidget(save_button)
        #central_widget.setLayout(layout_anavis)
        self.setCentralWidget(central_widget)  # 设置窗口的中心小部件
        self.show()  # 显示窗口
        #self.setLayout(vbox)
        #central_widget.show()
