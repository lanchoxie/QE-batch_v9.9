            ...
            if (self.mode=="pdos")&(data[i][-2].find("DONE")!=-1):
                ...
                for read_ele_lines in f_read_ele:
                    if read_ele_lines.find("ATOMIC_SPECIES")!=-1:
                        ...
                        start_reading=0
                    if start_reading==0.5:
                       start_reading=1
                       continue
                    if start_reading==1:
                        elements["el"+str(i)].append([x for x in read_ele_lines.split() if len(x)>0][0])
                #print("readed from scf",elements["el"+str(i)])
                #print(table.item(i, 0).text())
                hbox_pdos["hbx"+str(i)] = QHBoxLayout()
                hbox_pdos["hbx"+str(i)].addStretch()
                for ele in range(len(elements["el"+str(i)])):
                    checkBox["chbx"+str(i)]=QCheckBox(elements["el"+str(i)][ele])
                    hbox_pdos["hbx"+str(i)].addWidget(checkBox["chbx"+str(i)])

                widget_pdos["wg"+str(i)] = QWidget()
                widget_pdos["wg"+str(i)].setLayout(hbox_pdos["hbx"+str(i)])
                table.setCellWidget(i, table.columnCount()-2, widget_pdos["wg"+str(i)])
                #print("before button",elements["el"+str(i)])
                button.clicked.connect(lambda state,x=i:self.button_click_pdos(button,table,button_clicked,x,elements["el"+str(x)],hbox_pdos["hbx"+str(x)],checkBox["chbx"+str(x)],lineEdit_x_l,lineEdit_x_r,lineEdit_y_l,lineEdit_y_r))
                table.setCellWidget(i, table.columnCount()-1, button)
...
#def copy button
    ...
    def button_click_pdos(self,button,table,button_clicked,x,elements,hbox_pdos,checkBox,lineEdit_x_l,lineEdit_x_r,lineEdit_y_l,lineEdit_y_r):
        str_name=table.item(x, 0).text().strip()
        #print("in button clicked",elements)
        if button_clicked[x]==False:
            os.chdir(str_name)
            for ele in range(len(elements)):
                if (os.path.isfile("accumulated_pdos_file%s"%elements[ele])==0)&(elements[ele]!="total"):
                    print("calculating :%s"%elements[ele])
                    data_pdos=os.popen("python %s/sum_qe_pdos.py %s"%(code_path,elements[ele])).readlines()
            os.chdir(Current_dir)
            button=self.sender()
            button.setText("查看PDOS")
            button_clicked[x] = True
            self.pdos_show(str_name,self.get_selected_element(elements,hbox_pdos,x,checkBox),lineEdit_x_l,lineEdit_x_r,lineEdit_y_l,lineEdit_y_r)
        elif button_clicked[x]==True:
            self.pdos_show(str_name,self.get_selected_element(elements,hbox_pdos,x,checkBox),lineEdit_x_l,lineEdit_x_r,lineEdit_y_l,lineEdit_y_r) 

    def pdos_show(self,str_name,elements_sel,lineEdit_x_l,lineEdit_x_r,lineEdit_y_l,lineEdit_y_r):
            ...
            if "total" in elements_sel:
                ...
                out_pdos="\"%s/BTO.pdos_tot\" u ($1-%f):3 w l t \"Total\","%(str_name,Fermi_E)
            ...
            for ele in range(len(elements_sel_ele)):
                out_pdos+="\"%s/accumulated_pdos_file%s\" u 1:3 w l t \"%s\","%(str_name,elements_sel_ele[ele],elements_sel_ele[ele])
