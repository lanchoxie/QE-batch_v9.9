...
db_path = 'subgraphs_5_v6_coordinates_concentration_42d.db'
...
def networkx_to_pyg_graph(subgraph):
    ...
    for u, v, data in subgraph.edges(data=True):
        u_vec=subgraph.nodes[u]['feature'][-12:]
        v_vec=subgraph.nodes[v]['feature'][-12:]
    ...
    # 提取节点特征
    node_features = [subgraph.nodes[node]['feature'][:-12]for node in subgraph.nodes()]
    # 创建归一化器实例
    scaler = MinMaxScaler()
    # 拟合数据并转换
    ...
    for u, v, data in subgraph.edges(data=True):
        if data.get('edge_type') == 'li_ni_edge':
            exchange_energy = data.get('exchange_E', None)
            ...
            for node in subgraph.nodes():
                if node != u and node != v:  # 排除u和v自身
                    ...
                    if node in k_hop_neighbors_u:
                        u_vec = subgraph.nodes[u]['feature'][-12:]
                        node_vec = subgraph.nodes[node]['feature'][-12:]
    ...
    # 将能量信息转换为 PyTorch 张量
    y = torch.tensor([exchange_energy*13.605], dtype=torch.float)
...
if debug==0:
    ...
    # 保存模型的状态字典
    print("Saving models...")
    torch.save(model.state_dict(), f'model_GAT_train-valence-Resnet-aggregate_k2-FC3_4_v4_coordinates_R2_{r2:.3f}.pth')
    ...
    os.system(f"mkdir {test_r2:.3f}_batch_{batch_size}_FC_4_heads_{gat_heads}_seed_{seed}")
    os.system(f"mv *.png {test_r2:.3f}_batch_{batch_size}_FC_4_heads_{gat_heads}_seed_{seed}")
    os.system(f"mv model* {test_r2:.3f}_batch_{batch_size}_FC_4_heads_{gat_heads}_seed_{seed}")
    os.system(f"cp {sys.argv[0]} {test_r2:.3f}_batch_{batch_size}_FC_4_heads_{gat_heads}_seed_{seed}")
