...
if os.path.isfile("%s/flat.save/FLAT_SAVE"%code_path)==0:
    #print("AAA")
    os.mknod("%s/flat.save/FLAT_SAVE"%code_path)
    f_initializing=open("%s/flat.save/FLAT_SAVE"%code_path,"w")
    f_initializing.writelines("#flat_number  queue_name  type_flat  ppn_num_defaut\n")
    f_initializing.writelines("1  zf_normal  pbs  52\n")
    f_initializing.writelines("2  spst_pub  pbs  32\n")
    f_initializing.writelines("3  dm_pub_cpu  slurm  32\n")
    f_initializing.close()
...
def read_flat_state():
    ...
    flat_save=read_files("%s/flat.save/FLAT_SAVE"%code_path)
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        flat_save=read_files("%s/flat.save/FLAT_SAVE"%code_path)
        flat_types_list=[i[1] for i in flat_save]
        for i_type in flat_types_list:
            self.combo_flat.addItem(i_type)
    ...
    def flat_save(self,ppn_lineedit,node_lineedit,wallT_lineedit):
        ...
        #flat_number,node_num,ppn_num,wall_time,type_flat=read_flat_state()
        flat_save=read_files("%s/flat.save/FLAT_SAVE"%code_path)
        flat_ind=[i[1] for i in flat_save].index(type_flat_new)
        type_flat=flat_save[flat_ind][1]
        type_hpc=flat_save[flat_ind][2]
        ppn_num=int(flat_save[flat_ind][3])

        print(type_flat_new,node_num_new,ppn_num_new,wallT_new)
        flat_number_new=int(flat_save[flat_ind][0])
