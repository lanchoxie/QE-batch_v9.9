...
working_directory = "C:\\Users\\xiety\\Desktop\\da-CGAT\\supercell_create\\Doping_out\\" # 或者你可以设置为任何你想要处理 VASP 文件的目录
dir_merge=working_directory+"combined_vasp_files"
xsf_dir=working_directory+"combined_xsf_files"
os.makedirs(xsf_dir,exist_ok=True)

def process_all_vasp_files(directory):
...
def main_fun(str_name_tot):
    ...
    f1=open(f"{xsf_dir}/"+outname,"w+")
    for i in output:
        f1.writelines(i)    
    print(f"{xsf_dir}/{outname} created!")

if __name__=="__main__":
    process_all_vasp_files(dir_merge)