...
#str_file_dir:
str_file = f"{str_in}/out_relax_{str_in}.xsf"
...
if os.path.isfile(f"data.save/{str_in}_{clustering_element}_NB-{str(ele_neighbor)}_{octahedral_clustering}.pkl"):
    ...
    clustering_ele_lst=[]
    for i in structure:
        if i.species!=Composition(cls_lst[0]):
    ...
    if octahedral_clustering==0:
        ...
        print(data)
    max_clusters = max_cls_num if len(data)>max_cls_num else len(data)
...
process1 = subprocess.Popen(["python", f"{code_dir}/show_fig.py", f"data.save/Elbow_Rule_{str_in}_{clustering_element}_NB-{str(ele_neighbor)}_{octahedral_clustering}.png"])
# 第二张图：数据点的分布
#plt.subplot(1, 2, 2)
plt.figure(figsize=(6, 5))
...
os.system(f"python {code_dir}/show_fig.py data.save/Point_distribute_{str_in}_{clustering_element}_NB-{str(ele_neighbor)}_{octahedral_clustering}.png")
